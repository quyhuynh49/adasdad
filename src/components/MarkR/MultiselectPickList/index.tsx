...
export default function Index({
  ...
  >(
    data.reduce(
      (obj, datafield) => ({ ...obj, [datafield.data_field_1]: false }),
      {}
    )
  );
  ...
  const handleConfirm = () => {
    ...
    setResults(
      data.filter((el) =>
        Object.entries(selectedDataFields)
          .filter((el) => el[1] === true)
          .map((el) => el[0])
          .includes(el.data_field_1)
      )
    );
    /* reset search text */
    setSearch("");
  ...
  return (
    <div
      className={clsx(
        "relative w-full h-full rounded-[15px] p-[10px]",
        className,
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsDropdownDisplayed((preState) => !preState)}>
          <DropdownIcon />
        </button>
      </div>
      <button
        ...
        className={clsx(
          "w-full mt-[16px] font-sst font-[500] text-[16px] flex flex-col gap-[5px]",
      ...
      {isDropdownDisplayed && (
        <div className="z-10 absolute w-[180%] border border-solid border-[#999999] bg-[#ffffff] shadow-lg p-[10px]">
          <input
            className="w-full bg-[#EEEEEE] text-[#C2C2C2] font-[500] rounded-[5px] font-sst focus:outline-none px-[10px] py-[5px]"
            value={search}
            ...
            {data
              .filter((el) =>
                search.trim().length === 0
                  ? el
                  : el.data_field_1.toLowerCase().includes(search.trim())
              )
              .map((datafield) => {
                return (
                  <div
                    key={datafield.data_field_1}
                  ...
                  >
                    ...
                    />
                    <label
                      onClick={(event) => handleOnClickChecked(event)}
                      className="font-sst font-[700]"
                      htmlFor={`input-${datafield.data_field_1}`}
                    >
          ...
          <div className="flex flex-wrap w-full justify-end gap-[10px]">
            <button
              ...
              className="w-[150px] bg-[#BB33FF] text-[#ffffff] px-[16px] border rounded-[30px] font-sst font-[700] py-[6px]"
            >
              CONFIRM
            </button>
            <button
              onClick={() => setIsDropdownDisplayed((preState) => !preState)}
              className="w-[150px] bg-[#ffffff] text-[#BB33FF] px-[16px] border rounded-[30px] font-sst font-[700] py-[6px] border-[#BB33FF]"
            >
              CANCEL
            </button>
          </div>
...
const MAPPING_MULTISELECT_PICKLIST_WRAPPER_STYLE_DEFAULT: {
  [key: string]: string;
} = {
  main: "bg-[#F2F7FF]",
  secondary: "bg-[#F2F2F2]",
